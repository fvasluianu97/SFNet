...
from dwnet import DWT, DWT_transform
...
class CAM(nn.Module):
    def __init__(self, num_channels, compress_factor=4):
...
class BlockRGB(nn.Module):
    def __init__(self, in_ch, out_ch, k_sz=3, dropout_prob=0.5, device="cuda"):
        ...
        self.conv1 = nn.Conv2d(in_ch, out_ch, k_sz, padding=k_sz // 2, padding_mode="reflect", bias=True)
        self.op1 = nn.LeakyReLU(0.2)
        self.dyndc = SimplifiedDepthwiseCA(num_channels=out_ch, k=13, K=4, device=device)
        self.conv2 = nn.Conv2d(out_ch, out_ch, k_sz, padding=k_sz // 2, padding_mode="reflect", bias=True)
        self.op2 = nn.LeakyReLU(0.2)

        self.rconv1 = nn.Conv2d(in_channels=in_ch, out_channels=out_ch, kernel_size=1, padding=0, stride=1,
                                groups=1, bias=True)
...
class SPFBlockDown(nn.Module):
    def __init__(self, in_size, rgb_in_size, out_size, dwt_size=1, dropout=0.0, default=False, device="cuda"):
        super().__init__()
        ...
        if dwt_size > 0:
            self.dwt = DWT_transform(in_channels=in_size, out_channels=dwt_size)
        ...
        self.fp = FusedPooling(nc=out_size, blend=False)
...
class SPFAM(nn.Module):
    ...
    def __init__(self, cl, cr):
        super().__init__()
        self.scale = (0.5 * (cl + cr)) ** -0.5

        self.norm_l = LayerNorm2d(cl)
        self.l_proj_res = nn.Conv2d(cl, cl // 2, kernel_size=1, stride=1, padding=0)
        self.r_proj_res = nn.Conv2d(cr, cl // 2, kernel_size=1, stride=1, padding=0)

        self.l_proj1 = nn.Conv2d(cl, cl, kernel_size=1, stride=1, padding=0)
        self.r_proj1 = nn.Conv2d(cr, cl, kernel_size=1, stride=1, padding=0)

        self.beta = nn.Parameter(torch.zeros((1, cl // 2, 1, 1)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, cl // 2, 1, 1)), requires_grad=True)

        self.l_proj2 = nn.Conv2d(cl, cl // 2, kernel_size=1, stride=1, padding=0)
        self.r_proj2 = nn.Conv2d(cr, cl // 2, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x_l, x_r):
        ...
        return torch.cat((self.l_proj_res(x_l) + F_r2l, self.r_proj_res(x_r) + F_l2r), dim=1)


class SPFBlockUp(nn.Module):
    def __init__(self, in_size, rgb_size, dwt_size, out_size, dropout):
        ...
        self.rgb_proj = nn.ConvTranspose2d(in_channels=rgb_size, out_channels=out_size, kernel_size=4, stride=2,
                                           padding=1)
        if dwt_size > 0:
            self.spfam = SPFAM(rgb_size, dwt_size)
...
class SPFNet(nn.Module):
    def __init__(self, in_channels, device="cuda"):
        ...
        self.kab = knowledge_adaptation_convnext()

        self.c1 = SPFBlockDown(in_size=in_channels, rgb_in_size=3,
                               out_size=64, dwt_size=1, dropout=0.15, default=True, device=device)
        self.c2 = SPFBlockDown(in_size=129, rgb_in_size=64, out_size=128, dwt_size=2, dropout=0.2, device=device)
        self.c3 = SPFBlockDown(in_size=258, rgb_in_size=128, out_size=256, dwt_size=4, dropout=0.25, device=device)
        self.c4 = SPFBlockDown(in_size=516, rgb_in_size=256, out_size=512, dwt_size=8, dropout=0.3, device=device)
        self.c5 = SPFBlockDown(in_size=1032, rgb_in_size=512, out_size=1024, dwt_size=16, dropout=0.0, device=device)

        self.d5 = SPFBlockUp(in_size=2064, dwt_size=16, rgb_size=1024, out_size=512, dropout=0.0)
        self.d4 = SPFBlockUp(in_size=1024, dwt_size=8, rgb_size=512, out_size=256, dropout=0.3)
        self.d3 = SPFBlockUp(in_size=512, dwt_size=4, rgb_size=256, out_size=128, dropout=0.25)
        self.d2 = SPFBlockUp(in_size=256, dwt_size=2, rgb_size=128, out_size=64, dropout=0.2)
        self.d1 = SPFBlockUp(in_size=128, dwt_size=1, rgb_size=64, out_size=in_channels, dropout=0.1)
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(self.out_conv(torch.cat((y1, self.kab(x_rgb)), dim=1)))


if __name__ == '__main__':
    inp = torch.rand((1, 3, 512, 512))
    spfnet = SPFNet(16, device="cpu")
    out = spfnet(inp)
    print(out.shape)