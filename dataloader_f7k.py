...
class Image_Pair_Loader(data.Dataset):
    def __init__(self, dataroot_gt, dataroot_lq, gt_size=(512, 512)):
        super(Image_Pair_Loader, self).__init__()
        self.gt_folder, self.lq_folder = dataroot_gt, dataroot_lq
        self.paths = glod_from_folder([self.lq_folder, self.gt_folder], ['lq', 'gt'])
        self.to_tensor = transforms.ToTensor()
        self.gt_size = gt_size
        self.transform = transforms.Compose([transforms.Resize(self.gt_size), transforms.ToTensor()])
        print("Sucessfully loaded {} validation samples".format(len(self.paths['lq'])))
    ...
    def load_additional_samples(self, dataroot_gt, dataroot_lq):
        index = glod_from_folder([dataroot_lq, dataroot_gt], ['lq', 'gt'])
        print("Loaded {} additional input files and {} gt files".format(len(index['lq']), len(index['gt'])))
        self.paths["lq"] += index["lq"]
        self.paths["gt"] += index["gt"]
...
if __name__ == '__main__':
    ...
    flare_image_loader=Flare_Image_Loader('data/Flickr24K',transform_base,transform_flare)
    flare_image_loader.load_scattering_flare('data/Flare7Kpp/Flare7K','data/Flare7Kpp/Flare7K/Scattering_Flare/Compound_Flare')
    flare_image_loader.load_reflective_flare('Flare7K','data/Flare7Kpp/Flare7K/Reflective_Flare')

    val_loader  = Image_Pair_Loader("data/Flare7Kpp/test_data/real/gt",
